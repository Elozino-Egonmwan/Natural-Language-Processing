...
for regularization in REGULARIZATION:
    ...
    weights = sgd(lambda weights: softmax_wrapper(trainFeatures, trainLabels, 
        weights, regularization), weights, 3.0, 1000, PRINT_EVERY=1000000)
...
for i in range(5):
    if i == 0:
        print "%d = VERY NEGATIVE (<=0.2)" % i
    elif i == 1:
        print "%d = NEGATIVE (<= 0.4)" % i
    elif i == 2:
        print "%d = NEUTRAL (<= 0.6)" % i
    elif i == 3:
        print "%d = POSITIVE (<= 0.8)" % i
    else:
        print "%d = VERY POSITIVE (> 0.8)" % i
...
for idx in range(7):    
    words, _ = testset[idx]
    print "%d   |    %d  | %s" % (pred[idx],testLabels[idx],words)
