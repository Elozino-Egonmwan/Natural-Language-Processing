<?php echo'
<form class="form-horizontal" role="form" name="employeeDir" method="post" enctype="multipart/form-data" action="UpdateEmployeeDB.php?id='.$id.'" target="formFrame" id="form'.$id.'"> ';?> 
		...
		<table class="table table-hover staffTable table-condensed col-sm-12" id = "staffTable'.$id.'">  ';?>      
			<tbody> 
				<tr> 
					<td> <!--image --> 
                    	<?php echo'			
						<img src='.$src.' class="img-thumbnail" alt="No Photo" width="100" height="100" id = "fotoSrc'.$id.'" data-toggle="tooltip" data-placement="right" title="'.$foto.'"><br><br>';
						$fotoID = "'foto$id'";
						$imgSrc = "'fotoSrc$id'";
						//$y = "$x";
						echo'
						<input type = "button" value = "Upload" onClick = "hideImage('.$fotoID.')"> 
						<input type="file" name="fileToUpload"  id = "foto'.$id.'" onchange="imgPreview(this,'.$imgSrc.','.$savedFoto.','.$id.',id)" style="display:none" >
					...
					<td><!-- main form details: Email,PrefixTitle,FirstNme,Last name, Postfix,Function,Grp, rm number, rm directions -->								
						...
						<div class="form-group">				  
							...
							<div class="col-sm-6"><!--room directions --><?php echo'				    
                                <a href = "javascript:void(0)" onClick="directions(this,'.$savedRm.','.$id.')">&#8592;</a>&nbsp; &nbsp; 
                                <a href = "javascript:void(0)" onClick="directions(this,'.$savedRm.','.$id.')">&#8593;</a>&nbsp; &nbsp;
                                <a href = "javascript:void(0)" onClick="directions(this,'.$savedRm.','.$id.')">&#8594;</a>&nbsp; &nbsp;
                                <a href = "javascript:void(0)" onClick="directions(this,'.$savedRm.','.$id.')">&#8595;</a>&nbsp;&nbsp; 
                                <a href = "javascript:void(0)" onClick="directions(this,'.$savedRm.','.$id.')">&#8598;</a>&nbsp;&nbsp;
						...
						<div class="form-group">
							<div class = "col-sm-12"> <!-- statuses--><?php
                            	$sql = "SELECT * FROM SSW_STAFF_STATUS WHERE StaffID = '$staffID' ORDER BY Stop ASC";
                                        $resultSubTable = $conn->query($sql);
										$i = 0; 			  
                                        if ($resultSubTable->num_rows == 0) { 
                                          include 'blankStatus.php' ;
                                        }else{?>			  
								   <table class="table table-hover table-bordered statusTable">        
									<tbody class="sortable1 connectedSortable">
                                    	<?php
                                    	 /* status table data */	   
                                        /*$sql = "SELECT * FROM SSW_STAFF_STATUS WHERE StaffID = '$staffID' ORDER BY Start DESC";
                                        $resultSubTable = $conn->query($sql);
										$i = 0; 			  
                                        if ($resultSubTable->num_rows == 0) {						 
                                          echo '<tr>';
                                          ...
                                          echo'</tr>'; $i++;
                                           ...
                                           while($row = $resultSubTable->fetch_assoc()) {
											...
											<tr> 
												<td><!--display--><?php echo'
													<p class="checkbox" style="margin-left:20px"><input type="checkbox" name = "display[]" value = "'.$status.'" id = "display'.$i.''.$id.'"  data-toggle="tooltip" data-placement="right" title="Display"  onchange = "updateDisplay(this,'.$s.')" '.$chkd.'><input type="hidden" id = "display'.$i.''.$id.'Checker"></p>'; ?>							
												</td>
												<td class="col-sm-4"><!-- status--> <?php echo'
                                                	<input type="text" name="s_Status[]" id = "status'.$i.''.$id.'" value = "'.$status.'" class="form-control" placeholder="Status" onkeyup="saveState(this,'.$savedStatus.',id,'.$id.')" onClick = "attemptManualStatusChange()" readonly><input type="hidden"  id = "status'.$i.''.$id.'Checker">';?>
                                                </td>
                                                <!-- calendar addon -->
												<td class="col-sm-4"><!-- start date--><?php echo'							
													
    												<input type="text" class="form-control" id = "start'.$i.''.$id.'" value = "'.$start.'" placeholder="From" name= "startDate[]"  onkeyup="saveState(this,'.$savedStart.',id,'.$id.')" onClick = "attemptManualStatusChange()" readonly><input type="hidden" id = "start'.$i.''.$id.'Checker"> '; ?>
    													
																				
												</td>				
												<td class="col-sm-4"><!-- end date--><?php echo'
													
    													<input type="text" class="form-control" id = "stop'.$i.''.$id.'"  value = "'.$stop.'" placeholder="To" name= "endDate[]" onkeyup="saveState(this,'.$savedStop.',id,'.$id.')" onClick = "attemptManualStatusChange()" readonly><input type="hidden" id = "stop'.$i.''.$id.'Checker">';?>
    													
												</td>
                                                <!-- delete  and add status functions removed -->
												
											</tr><?php $i++; }; //close while  ?>
										</tbody>
	...
	</div> <!--close table responsive  <span class="glyphicon glyphicon-trash trash" id = "trash'.$id.'"  alt="Delete Staff"></span>-->	
</form>